...
|---|---------|-------|----------|
| 1 | **Path traversal** on `/api/load_scenario` (name unsanitized → arbitrary `*.json` read), reachable on the public HF Space | `scripts/manoeuvre_ihm.py:1991` → `:1208` | Security |
| 2 | **MultiDiGraph edge cache keyed by 2-tuples** — parallel circuits (ubiquitous on the RTE grid) silently overwrite each other in flow-influence caches | `action_evaluation/discovery/_base.py:775-781` | Correctness |
| 3 | **Unbounded pypowsybl variant leak** — `keep_variant=True` observations are never released by any caller; long-running UI service accumulates C++-side state per candidate | `pypowsybl_backend/observation.py:762-765`, `utils/simulation_pypowsybl.py:177`, `utils/reassessment.py:165` | Resource leak |
| 4 | Library code calls **`sys.exit(0)`** on load-flow divergence — terminates the host process (UI/HTTP callers) with a *success* code | `environment.py:233-235`, `environment_pypowsybl.py:342-344` | Robustness |
| 5 | `utils/load_training_data.py` / `load_evaluation_data.py` contain **hard bugs** (indexing a `StateInfo`, `NameError` on a `__main__`-only global, `raise("string")`, a "reconnect" action that disconnects) | see §4.4 | Correctness |
...
1. ~~C-diag~~ **Done in this branch**: `check_rho_reduction` now passes `obs_baseline` into the with-baseline call (`simulation_pypowsybl.py:230-233`), fixing the five topological mixins; the three injection mixins remain, folded into R4.
